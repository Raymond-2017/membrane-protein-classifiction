...
class ModelHepler:
    ...
    def get_callback(self, use_early_stop=True, tensorboard_log_dir=None,
...
MODEL_NAME = 'BiLSTM-TextCNN-epoch-30-emb-20'
...
# x_test = np.pad(x_test, ((0, 0), (0, maxlen-x_test.shape[1]), (0, 0)), 'constant')
# x_test = np.array([pad_sequences(seq, maxlen=maxlen, padding='post') for seq in x_test])
    ...
    for batch_size in batch_sizes:
        for units in LSTM_units:
            for filter in conv1d_filters:
                ...
                plt.title('(b) Plain BiLSTM-TextCNN')
